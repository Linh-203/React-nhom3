...
import { useDetailOrderQuery, useUpdateOrderMutation } from '../../api-slice/orderAPI'
...
const OrdersDetail = () => {
  ...
  <div>  
    ...
    <div>
    <strong className="font-semibold">Thanh toán: {detailOrder?.pay ? "Đã thanh toán" :"Chưa thanh toán"}</strong>
    <select className="mt-2 border border-gray-300 rounded-md p-1" onChange={(e)=>setPay(e.target.value)}>
      <option hidden value={detailOrder?.pay}> {detailOrder?.pay ? "Đã thanh toán" :"Chưa thanh toán"}</option>
    ...
    <div>
    <strong className="font-semibold">Dự kiến ngày nhận: {detailOrder?.receivedDate} <br /><input type="date" value={receivedDate} onChange={(e)=>setDate(e.target.value)} /> </strong> 
  </div>
  <strong className="font-semibold">Trạng thái:</strong>
    <select className="mt-2 border border-gray-300 rounded-md p-1" onChange={(e)=>setStatus(e.target.value)}>
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center">
          ...
          <div className="ml-4">
            ...
            <p className="text-gray-500 text-red-500">${item.productId.price} X {item.quantity}</p>
          </div>
        </div>
