...
function AnalyticOverview({ className, total, resultPercent,  state, title }: IProps) {
   ...
   return <div className={`w-1/5 rounded-2xl ${className ? className : ''} relative p-5`}>
        {state === "Decrease" ? <GrownDown className="absolute top-3 right-3"/> : <GrownUp className="absolute top-3 right-3" />}
        <h1 className="text-[35px] font-bold text-center">{numberRevert}</h1>
        <h5 className="text-center">{title}</h5>
        <p className="text-[13px] text-center text-gray-400">{resultPercent}% {state.length == 2 ? state[0]: state} from Last Week</p>
   </div>;
}
